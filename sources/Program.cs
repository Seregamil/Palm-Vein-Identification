...
namespace Biometrics.Palm
{
    ...
    {
        public static List<UserModel> listOfUsers = new List<UserModel>();
        public static List<PalmModel> listOfPalms = new List<PalmModel>();

        static void Main(string[] args)
        {
            var listOfImages = Directory.GetFiles(Settings.imagesFolderPath, "*.jpg").ToList();
            ...
            //? get list of all filenames without .jpg and generate cntr listOfPalms
            listOfImages.ForEach(x =>
            {
                ...
                listOfPalms.Add(new PalmModel() { 
                    ...
                    Owner = owner,
                    FileName = x,
                    Directory = $"{Settings.imagesOutputDirectory}/{owner}"
                });
            });

            //? get names and create name collection
            //? create dirs for saving data
            var userNames = listOfPalms.DistinctBy(x => x.Owner);
            userNames.ForEach(x => {
            ...
            var ROITask = Task.Factory.StartNew(() => 
            {
                listOfPalms.ForEach(x =>
                {
                    ...
                    {
                        var path = $"{Settings.imagesFolderPath}/{x.FileName}.jpg";
            ...
            //! Create dump
            Dump.CreateROI(Settings.binaryROIDirectory, listOfPalms);

            //! Uncomment next block for loading images from dump and comment all lines before this
            // listOfPalms = Dump.LoadROI(Settings.binaryROIDirectory, listOfImages);
            // var countFrom = 0;
            // var countTo = listOfPalms.Count;
            // ------------------------------
            ...
            var filtersTask = Task.Factory.StartNew(() => 
            {
                listOfPalms.ForEach(x =>
                {
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        var owner = listOfUsers.Find(u => u.Name == x.Owner);
            ...
            listOfUsers.ForEach(x => {
                ...
                for (var oX = 0; oX != patternSize.Width; oX++)
                {
                    for (var oY = 0; oY != patternSize.Height; oY++)
                    {
                        ...
                        foreach (var palm in x.Patterns)
                        { // 255 or 0
                            var px = palm.Get<byte>(oX, oY);

                            if (px == Settings.COLOR_WHITE)
                            { // white
